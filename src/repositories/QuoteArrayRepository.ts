...
export default class QuoteKnexRepository implements QuoteRepository {
  ...
  public async create(
    params: QuoteRepository.CreateParameters
  ): Promise<Quote> {
    logger.debug(`${this.constructor.name}.create`, { params });

    const id = quotesList.length + 1;
    const quote = { ...params, id, createdAt: new Date().toISOString() };
  ...
  public async update(
    params: QuoteRepository.UpdateParameters
  ): Promise<Quote> {
    logger.debug(`${this.constructor.name}.update`, { params });

    const quote = quotesList.find((q) => q.id === params.id);
